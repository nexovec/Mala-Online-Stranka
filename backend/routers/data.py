...
def get_spider(
    ...
    vol_score = vol_score / pocet_obyvatel.loc[pocet_obyvatel["koduzemi"] == city, "hodnota"].values[0]
    ...
    prir_score = prir_score / pocet_obyvatel.loc[pocet_obyvatel["koduzemi"] == city, "hodnota"].values[0]
    # lesní půda
    # zeleň
    ...
    vzdelani_score = vzdelani_score / pocet_obyvatel.loc[pocet_obyvatel["koduzemi"] == city, "hodnota"].values[0]
    # materske skoly
    # zakladni skoly
    ...
    bydleni_score = bydleni_score / pocet_obyvatel.loc[pocet_obyvatel["koduzemi"] == city, "hodnota"].values[0]
    # byty

    zdravotnictvi = [70300, 71000, 70200]
    zdravotnictvi_df = df.loc[df["kodukaz"].isin(zdravotnictvi)]
    zdravotnictvi_score = zdravotnictvi_df.loc[zdravotnictvi_df["koduzemi"] == city, "hodnota"].sum()
    zdravotnictvi_score = zdravotnictvi_score / zdravotnictvi_df["hodnota"].max()
    zdravotnictvi_score = zdravotnictvi_score / pocet_obyvatel.loc[pocet_obyvatel["koduzemi"] == city, "hodnota"].values[0]
    # nemocnice
    # lékarny
    # zdravotni strediska
    ...
    df["values"] = (df["values"] - df["values"].min()) / (df["values"].max() - df["values"].min())
